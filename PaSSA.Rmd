...
Let's calculate the sample size for our simple experiment. For this we'll use the {pwr} package. The `pwr.p.test()` function can be used to calculate power or sample size for a one-sample proportion test.
...
We can also estimate sample size based on a desired precision. Say we hypothesize the difference in proportions between males and females is 0.05 and we want to estimate a 95% confidence interval within 0.03 margin of error. Further assume one group is 0.50 and the other 0.55. That corresponds to a width of 0.06. We can use the `prec_riskdiff()` function from the presize package to estimate the sample size.
...
We can also estimate sample size based on a desired precision. Let's say we want to estimate the _difference in means_ using a 95% confidence interval with a margin of error of 50 cents. Again assume the true difference is 75 cents and the population standard deviation is 2.25. We can use the `prec_meandiff()` function. Notice we specify `variance = "equal"` since were assuming the standard deviation is the same for males and females.
...
```{r}
prec_meandiff(delta = 0.5, sd1 = 2.25, conf.width = 1)
```
...
40 subjects provides high power assuming the model really will explain 30% of the variation in job satisfaction.
