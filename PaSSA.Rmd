...
Load the packages we'll use in this workshop.
...
Let's calculate the sample size for our simple experiment:

I suspect people place sticky-back name tags on the left side of their chest about _75%_ of the time. What should my _sample size_ be if I want high probability of rejecting the null hypothesis (50%) assuming my alternative hypothesis (75%) is true? 

For this we'll use the pwr package. The `pwr.p.test()` function can be used to calculate power or sample size for a one-sample proportion test.
...
The `ES.h()` function transforms the absolute difference into an effect size that takes the _relative_ difference into account. The reason we have to transform the effect size for proportions is to distinguish between _absolute_ and _relative_ differences.

```{r}
ES.h(p1 = 0.75, p2 = 0.50)
```

Why do this? Notice the difference between absolute and relative differences in proportions.

The _absolute differences_ in these proportions are equivalent:

0.75 - 0.50 = 0.25
0.26 - 0.01 = 0.25

But the _relative differences_ are very different:

0.75/0.50 = 1.5  (ie, 0.75 is 1.5 times greater than 0.50)
0.26/0.01 = 26   (ie, 0.26 is 26 times greater than 0.01)

This means the effect sizes are very different.

```{r}
c(ES.h(p1 = 0.75, p2 = 0.50),
  ES.h(p1 = 0.26, p2 = 0.01))
```

The `ES.h()` function calculates an effect size comparing proportions that takes relative differences into account. 
...
- Accept and embrace the uncertainty in a power and sample size "analysis". It's a thought experiment.
...
```{r}
prop.test(x = 18, n = 23, p = 0.5, alternative = "greater", correct = FALSE)
```

Reject the null, but notice the wide 95% confidence interval.

## Sample size for precision

We can also estimate sample size based on a desired precision. Say we hypothesize the proportion of people who place a name tag on the left is 0.75, and we want to estimate that within 0.03 margin of error. That corresponds to a width of 0.06. We can use the `prec_prop()` function from the presize package to estimate the sample size.


```{r}
prec_prop(p = 0.75, conf.width = 0.06)
```

798. Notice how much bigger this sample size is! Precision requires a much larger sample size than simply reject a null hypothesis.

To avoid the warning. specify a method. The wilson method is suggested for small n (< 40), and the agresti-coull method is suggested for larger n (see reference).

```{r}
prec_prop(p = 0.75, conf.width = 0.06, method = "wilson")
```

```{r}
prec_prop(p = 0.75, conf.width = 0.06, method = "agresti")
```

If you have no idea of a hypothesized proportion, you can specify 0.5. That's the most conservative.

```{r}
prec_prop(p = 0.5, conf.width = 0.06, method = "agresti")
```

Most election polling samples are based on this calculation.
...
(4) How large a sample do we need to estimate a 95% confidence interval within a margin of 0.05 if we hypothesize the true proportion to be 0.75?
...
- High precision requires even larger sample sizes


# Two-sample proportion test

Let's say I plan to randomly survey male and female UVA undergrad students and ask them if they consume alcohol at least once a week. 

- null hypothesis: no difference between males and females that answer yes. 
- alternative hypothesis: there is a difference

This is "two-sided" alternative; one group has higher proportion, I don't know which.
...
```{r}
pwr.2p.test(h = ES.h(p1 = 0.20, p2 = 0.25), 
            sig.level = 0.05, power = 0.90)
```

Notice that n is the number in _each group_! 

The base R function does not require us to use a separate effect size function.

```{r}
power.prop.test(p1 = 0.20, p2 = 0.25, 
                sig.level = 0.05, power = 0.90)
```
...
```{r}
pwr.2p.test(h = ES.h(p1 = 0.05, p2 = 0.10), 
            sig.level = 0.05, power = 0.90)
```
...
```{r}
power.prop.test(p1 = 0.05, p2 = 0.10, 
                sig.level = 0.05, power = 0.90)
```

The nice thing about the pwr function is that it has a plot method.

```{r}
res <- pwr.2p.test(h = ES.h(p1 = 0.05, p2 = 0.10), 
            sig.level = 0.05, power = 0.90)
plot(res)
```
...
Let's say we sample 585 males and 567 females. 307 and 287 answer "yes", respectively, to consuming alcohol once a week.

The analysis:

```{r}
prop.test(x = c(307, 287), n = c(585, 567), correct = FALSE)
```

We fail to reject the null of no difference.

## Sample size for precision

We can also estimate sample size based on a desired precision. Say we hypothesize the difference in proportions between males and females is 0.05 and we want to estimate a 95% confidence interval within 0.03 margin of error. That corresponds to a width of 0.06. Further assume one group is 0.50 and the other 0.55. We can use the `prec_riskdiff()` function from the presize package to estimate the sample size.

```{r}
prec_riskdiff(p1 = 0.50, p2 = 0.55, conf.width = 0.06)
```

Need about 2120 _per group_.

## CODE ALONG 2

Add new R code chunks by pressing *Ctrl+Alt+I* (Win/Linux) or *Cmd+Option+I* (Mac). 

1) What sample size do we need for each group if assume proportions to be 0.50 and 0.55? Again assume we desire power to be 0.90 and a significance level of 0.05.

```{r}
power.prop.test(p1 = 0.5, p2 = 0.55, power = 0.9)
```

2) How large a sample do we need to estimate a 95% confidence interval on the difference of proportions within a margin of error 0.06 if we hypothesize the true proportions to be 0.45 and 0.55?

```{r}
prec_riskdiff(p1 = 0.45, p2 = 0.55, conf.width = 0.12, method = "agresti")
```
...
I'm interested in learning if there is a difference in the mean price of what male and female students pay at the library coffee shop. Let's say I plan to randomly observe 30 male and 30 female students check out from the coffee shop and note their total purchase price. How powerful is this experiment if I want to detect a difference of 75 cents in either direction?
...
`pwr.t.test()` function also has a plot method.


```{r}
res <- pwr.t.test(d = 0.75/2.25, sig.level = 0.05, power = 0.90)
plot(res)
```
...
We can also estimate sample size based on a desired precision. Let's say we want to estimate the _difference in means_ using a 95% confidence interval with a margin of error of 50 cents. Again assume the true difference is 75 cents and the population standard deviation is 2.25. We can use the `prec_meandiff()` function. Notice we specify `variance = "equal"` since we're assuming the standard deviation is the same for males and females.

```{r}
prec_meandiff(delta = 0.75, sd1 = 2.25, conf.width = 1, variance = "equal")
```

If we like we can assume different standard deviations.

```{r}
prec_meandiff(delta = 0.75, sd1 = 2.25, sd2 = 3, conf.width = 1, 
              variance = "unequal")
...
1) How many students should we sample for a power of 0.99 and a significance level of 0.001?


2) How large a sample do we need to estimate a 95% confidence interval on the difference of means within a margin of 50 cents if we hypothesize the true difference in means to be 1 dollar? Assume a standard deviation of 2.25.

```{r}
prec_meandiff(delta = 1, sd1 = 2.25, conf.width = 1)
```
...
To estimate a sample size for desired precision we need to focus on the difference in paired means. This reduces to a single value per subject. Say we want to estimate the difference in paired means with a 95% confidence interval with margin of error of 0.05. We can use the `prec_mean()` function. Assume the true difference in paired means is about -0.1 and that the standard deviation of the differences is 0.25. 

```{r}
prec_mean(mean = -0.1, sd = 0.25, conf.width = 0.10)
```

Notice there's no specifying "direction" when it comes to estimating sample size for precision.
...
1) How many pairs do we need to sample to achieve at least 0.95 power with a significance level of 0.01, assuming our hypothesized effect size is correct? Assume a difference of -0.1 and a standard deviation of 0.25.


2) How large a sample do we need to estimate a 95% confidence interval on the difference of paired means within a margin of 0.1 seconds if we hypothesize the true difference in paired means to be -0.1? Assume a standard deviation of 0.25.

```{r}
prec_mean(mean = -0.1, sd = 0.25, conf.width = 0.2)
```
...
I'm a web developer and I'm interested in 3 web site designs for a client. I'd like to know which design helps users find information fastest. I design an experiment where I will have 3 groups of randomly selected people use one of the designs to find some piece of information and I record how long it takes. (All groups look for the same information.) How many people do I need in each group if I believe two of the designs will take 30 seconds and one will take 25 seconds? Assume population standard deviation is 5 seconds and that I desire power and significance levels of 0.90 and 0.05, respectively.
...
```{r}
res <- pwr.anova.test(k = 3, f = sd_means*sqrt(2/3)/sd_pop, 
               sig.level = 0.05, power = 0.90)
plot(res)
```
...
One effect size measure for a one-way ANOVA is _eta-squared_. This measure represents an estimate of how much variance in the response variables is accounted for by the explanatory variable. We can estimate sample size based on the desired precision of eta-squared using the `size.ci.etasqr()` function from the statpsych package. 

"Set the planning value of eta-squared to about 1/3 for a conservatively large sample size." (from the help page) In other words, assume the explanatory variable explains about a 1/3 of the variation.

For 95% confidence interval set `alpha = 0.05`. Set `w` to desired CI width. Say we want to estimate eta-squared within 0.1 margin of error and we assume eta-squared is 1/3.

```{r}
statpsych::size.ci.etasqr(alpha = 0.05, etasqr = 1/3, groups = 3, w = 0.2)
```

Drawback of eta-squared: doesn't tell you how or which means differ. 
...
# Linear model/multiple regression: The F test

The one-way ANOVA is a special case of a linear model. In a linear model we estimate the mean of an outcome _conditional on multiple variables_.

Let's say I'm hired to survey a company's workforce about job satisfaction. I ask employees to rate their satisfaction on a scale from 1 (hating life) to 10 (loving life). I know there will be variability in the answers, but I think two variables that will explain this variability are salary and age. I could analyze this using a linear model/multiple regression. I would regress satisfaction on salary and age. 

The hypothesis test associated with multiple regression is the F test. Tests that the proportion of variance explained by the model predictors is 0. Equivalently, it tests whether all the model coefficients (except the intercept) are 0. (This test is reported at the bottom of the summary output for an `lm` object.)

The pwr function for this is `pwr.f2.test()`

A little tricky to use because not only do we have to supply an "effect size" (`f2`), we also have to supply numerator (`u`) and denominator (`v`) degrees of freedom instead of sample size. 

- The `f2` effect size is $R^2 / (1 - R^2)$, where $R^2$ is the coefficient of determination, aka the "proportion of variance explained".

- To determine effect size you hypothesize the proportion of variance your model explains, or the $R^2$. For example, 0.45. This leads to an effect size of 0.45/(1 - 0.45) = 0.81. We can also reverse this. Given an effect size, we can determine $R^2$:  $ES / (1 + ES)$. For example, 0.81/(1 + 0.81) = 0.45

- The numerator degrees of freedom, `u` is the number of coefficients you'll have in your model (minus the intercept).

- The denominator degrees of freedom `v` is the number of error degrees of freedom. This is basically your sample size minus the number of coefficients in your model, including the intercept: v = n - u - 1.

- if we want to determine sample size for a given power and effect size, we have to find `v`, which we then use to solve n = v + u + 1. There is no `n` argument!

Let's say I think my proposed model will explain at least 30% (R2 = .30) of the variation in job satisfaction. How powerful is my study if I randomly recruit 40 employees and accept a 0.05 significance level for the F test?

- u is number of predictors: 2 (age and salary)
- v is number of subjects minus model parameters: 40 - 3 = 37 (age, salary, intercept)
- f2 is the effect size, R2/(1 - R2): 0.3/(1 - 0.3)

```{r}
pwr.f2.test(u = 2, v = 37, f2 = 0.3/(1 - 0.3), sig.level = 0.05)
```

40 subjects provides high power (~0.95) assuming the model really will explain 30% of the variation in job satisfaction.

How many employees do I need to survey if I want to be able to detect at least 30% explained variance (R^2 = .30) with 80% power and the usual 0.05 significance level? We have to find v and then derive n.

```{r}
pwr.f2.test(u = 2, f2 = 0.3/(1 - 0.3), sig.level = 0.05, power = 0.8)
```

Round up v to 23. Now solve n, which is n = v + u + 1

```{r}
23 + 2 + 1
```

It's important to note that the alternative hypothesis here is that at least one of the coefficients in my model is not 0. This doesn't mean we need at least 26 subjects to have all coefficients significant. It just means 26 subjects gives us 80% chance of correctly detecting the effect of _at least one_ of our predictors, provided one or more truly affect our response.
...
Let's run it 1000 times and see what proportion of times we _detect_ the interaction at the significance level 0.01. Recall our sample size is 10 per group (or 40 overall).
...
https://www.jstor.org/stable/pdf/2685525


# Minimum sample size required for building a predictive model

A recent publication in the BMJ provides "guidance on how to calculate the sample size required to develop a _clinical prediction model_." The focus of the paper is on medical applications but the methods are applicable to any statistical analysis that involves _building_ a linear model. That is, you intend to model an outcome as a function of various _candidate predictor variables_, only some of which you may end up using.
